#!/usr/bin/env bash
# OPTIONAL / legacy data path: stream ShadowTraffic rows into a Polaris (Iceberg
# REST) *source* table. The current demo reads a local CSV instead, generated by
# scripts/generate_local_csv.sh — most colleagues will never need this script.
# Keep using it only if you want a live-appended Polaris source table. It requires
# POLARIS_* credentials (see scripts/setup_env.sh) and a ShadowTraffic license.
set -euo pipefail
